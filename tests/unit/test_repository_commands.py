...
class TestInitCommand:
    ...
    def test_init_creates_repository(
        self, mock_repo_class, mock_which, cli_runner, mock_root, tmp_config
    ):
        ...
        result = cli_runner.invoke(app, ["init", "--config", str(tmp_config)])
    ...
    @patch("kopi_docka.commands.repository_commands.shutil.which")
    def test_init_fails_without_kopia(self, mock_which, cli_runner, mock_root, tmp_config):
...
class TestRepoStatusCommand:
    ...
    def test_repo_status_shows_info(
        ...
        result = cli_runner.invoke(app, ["repo-status", "--config", str(tmp_config)])
...
class TestRepoWhichConfigCommand:
    ...
    @patch("kopi_docka.commands.repository_commands.KopiaRepository")
    def test_which_config_works(self, mock_repo_class, cli_runner, mock_root, tmp_config):
        ...
        result = cli_runner.invoke(app, ["repo-which-config", "--config", str(tmp_config)])
...
class TestRepoSetDefaultCommand:
    ...
    def test_set_default_creates_symlink(
        self, mock_path_class, mock_repo_class, cli_runner, mock_root, tmp_config
    ):
        ...
        result = cli_runner.invoke(app, ["repo-set-default", "--config", str(tmp_config)])
...
class TestRepoMaintenanceCommand:
    ...
    @patch("kopi_docka.commands.repository_commands.KopiaRepository")
    def test_maintenance_runs(self, mock_repo_class, cli_runner, mock_root, tmp_config):
        """repo-maintenance runs maintenance tasks."""
        mock_repo = mock_repo_class.return_value
        mock_repo.is_connected.return_value = True

        result = cli_runner.invoke(app, ["repo-maintenance", "--config", str(tmp_config)])
...
class TestRepoInitPathCommand:
    ...
    def test_init_path_requires_root(self, cli_runner, mock_non_root, tmp_path):
        ...
        result = cli_runner.invoke(app, ["repo-init-path", str(tmp_path / "repo")])
    ...
    def test_init_path_creates_repo(
        self, mock_repo_class, mock_run, cli_runner, mock_root, tmp_config, tmp_path
    ):
        ...
        result = cli_runner.invoke(
            app, ["repo-init-path", str(repo_path), "--config", str(tmp_config)]
        )
...
class TestRepoSelftestCommand:
    ...
    def test_selftest_creates_ephemeral(
        self, mock_config_class, mock_repo_class, cli_runner, mock_root, tmp_path
    ):
        ...
        result = cli_runner.invoke(app, ["repo-selftest", "--tmpdir", str(tmp_path), "--keep"])
