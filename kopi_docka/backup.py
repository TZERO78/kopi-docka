...
class BackupManager:
    ...
    def backup_unit(self, unit: BackupUnit, update_recovery_bundle: bool = None) -> BackupMetadata:
        ...
        Returns:
            BackupMetadata
        """
        logger.info(f"Starting backup of unit: {unit.name}", extra={'unit_name': unit.name})
        ...
        try:
            ...
            # 1) Stop containers
            logger.info(f"Stopping {len(unit.containers)} containers...", extra={'unit_name': unit.name})
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for volume in unit.volumes:
                    futures.append((volume.name, executor.submit(self._backup_volume, volume, unit, backup_id)))
            ...
            logger.info(f"Starting {len(unit.containers)} containers...", extra={'unit_name': unit.name})
        ...
        if metadata.errors:
            logger.warning(
                ...
                extra={'unit_name': unit.name, 'duration': metadata.duration_seconds, 'errors': len(metadata.errors)}
            )
        else:
            logger.info(
        ...
        for c in containers:
            if c.is_running:
                try:
                    ...
                    logger.error(f"Failed to stop container {c.name}: {e.stderr.decode()}",
    ...
    def _start_containers(self, containers: List[ContainerInfo]):
        ...
        for c in containers:
            try:
                subprocess.run(['docker', 'start', c.id], check=True, capture_output=True)
                logger.debug(f"Started container: {c.name}", extra={'container': c.name})
                self._wait_container_healthy(c, timeout=self.start_timeout)
            except subprocess.CalledProcessError as e:
                logger.error(f"Failed to start container {c.name}: {e.stderr.decode()}",
                             extra={'container': c.name})
    ...
    def _wait_container_healthy(self, container: ContainerInfo, timeout: int = 60):
        ...
        try:
            ...
            while time.time() - start < timeout:
                ...
                if status == 'healthy':
                    logger.debug(f"Container {container.name} is healthy", extra={'container': container.name})
                    return
                if status == 'unhealthy':
                    logger.warning(f"Container {container.name} is unhealthy", extra={'container': container.name})
                    return
                time.sleep(2)
            ...
            logger.debug(f"Health check failed for {container.name}: {e}", extra={'container': container.name})
            time.sleep(2)
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if unit.compose_file and unit.compose_file.exists():
                    (tmpdir / 'docker-compose.yml').write_text(unit.compose_file.read_text())
                ...
                for c in unit.containers:
                    raw = subprocess.run(
                        ['docker', 'inspect', c.id], capture_output=True, text=True, check=True
                    ).stdout
            ...
            logger.error(f"Failed to backup recipes for {unit.name}: {e}", extra={'unit_name': unit.name})
            return None
    ...
    def _save_metadata(self, metadata: BackupMetadata):
        ...
        logger.debug(f"Saved metadata to {metadata_dir / filename}", extra={'unit_name': metadata.unit_name})
    ...
    def _ensure_policies(self, unit: BackupUnit):
        ...
        for target in targets:
            try:
                ...
                subprocess.run(cmd, check=True, capture_output=True, text=True, env=self.repo._get_env())
