...
def create_default_config(path: Optional[Path] = None, force: bool = False) -> Path:
    ...
    # Find the template file - try multiple names for compatibility
    template_names = [
    ...
    if template_path and template_path.exists():
        ...
        if 'CHANGE_ME_TO_A_SECURE_PASSWORD' in template_content:
            ...
            logger.info(f"Generated secure password for repository")
        else:
            config_content = template_content
            
    else:
        # Fallback: Create minimal config without template
        logger.warning("Configuration template not found. Creating minimal config.")
        generated_password = generate_secure_password()
        config_content = f"""##############################################################################
