...
class KopiaRepository:
    ...
    def connect(self) -> None:
        """Connect to an existing repository; if not initialized, create then connect."""
        backend, args = self._detect_backend(self.repo_path)

        # Try connect first
        proc = self._run(["kopia", "repository", "connect", backend, *self._backend_args(backend, args)],
                         check=False)
        if proc.returncode == 0:
        ...
        needs_create = ("not initialized" in msg) or (backend == "filesystem" and not Path(args["path"]).exists())

        if needs_create:
            self.initialize()  # will create and ensure connection
            return

        # Otherwise, real failure (e.g., wrong password)
        raise RuntimeError(f"Failed to connect: {proc.stderr.strip() or proc.stdout.strip()}")

    def initialize(self) -> None:
        """
        Create new repository at repo_path and connect to it.
        ...
        # Create (exactly what Kopia expects)
        self._run([
        ...
        # Verify connected, otherwise connect explicitly
        try:
            _ = self.status(json_output=True, verbose=True)
        except Exception:
            self._run(["kopia", "repository", "connect", backend, *self._backend_args(backend, args)], check=True)
            _ = self.status(json_output=True, verbose=True)

        # Apply default/global policies from config (best-effort)
        try:
            from .policy import KopiaPolicyManager
